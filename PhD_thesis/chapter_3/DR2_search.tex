...
\begin{table}
 \centering
 \caption[List of objects that passed the initial visual inspections.]{List of objects that passed the initial visual inspections.}
 \resizebox{\textwidth}{!}{%Scale table to page width
...
\begin{table*}
 ...
 \begin{tabular}{llllcccc}
  \hline
  Primary name & IAU name & Sibling name & IAU name $^{(a)}$ & Type & Date $^{(b)}$ & Offset ($\arcsec$) $^{(c)}$\\
  \hline
...
\begin{sidewaystable}
 \centering
 \caption[Additional tests performed on the tem most promising objects.]{Results of the additional tests for the ten promising objects. The host separation is given in $\arcsec$~and converted to kpc using the redshift given in Table~\ref{33_list}. In the final five columns the similarity of each SN Ia with a late-time excess is compared to different transient classes to see if the late-time signal can be interpreted as another transient. This can be excluded based on an inconsistent colour (1), duration (2), and/or an excessive amount of host extinction (3) needed to obtain the observed magnitudes.}
 \resizebox{\textwidth}{!}{
