...
where $M_\text{ej}$ is the ejecta mass, $t$ is the time since explosion, $r$ is the distance to the explosion site, and $v_t$ is the transition velocity between the outer and inner ejecta, with typical values for SNe Ia being $v_t = \mathcal{O}(10^9\text{ cm s}^{-1})$. After the shock the CSM is compressed, heated, and starts emitting light. This shocked CSM is assumed to a be fully ionized \ion{H}-dominated gas with the electrons and ions in equipartition.
