...
%\color{red} This will be Paper III, but needs to be properly adapted. All commented out sections are still in the overleaf version if needed. Copied starting from Data until the end of conclusions, To-do list: Check for broken references, reformat pages, make sure all the \ztfg\ztfr\ztfi\ things are consistent\color{black}
...
\begin{sidewaystable*}[]
    \centering
    \caption{Follow-up observations of transients with late-time signals. The exposure time is noted as number of exposures x exposure time of a single exposure. Photometric observations taken in multiple bands with the same exposures are mentioned in the same row.}
    \resizebox{\textwidth}{!}{
